...
#
# The listing failure is deliberately NOT swallowed. `|| previous=''` would
# turn a transient API error into "there is no previous release for this ABI"
# and publish a duplicate of a set that is already out there -- the one thing
# this lookup exists to prevent. Under `set -e` the assignment simply fails.
#
# --limit is repo-global and unfiltered: gh has no server-side tag-prefix
# filter, so the window has to be wide enough to still contain this ABI's
# newest tag when several architectures publish independently.
previous=$(gh release list --limit 500 --json tagName,createdAt \
