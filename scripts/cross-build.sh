...
# Trimmed like the closure reader below trims: an indented origin would
# otherwise be handed to `cd` with its leading whitespace still attached and
# fail here as a missing directory rather than as a bad config line.
_probe="$(grep -v '^[[:space:]]*#' "$PKGLIST" | grep -v '^[[:space:]]*$' |
          head -n 1 | tr -d '[:space:]')"
...
visit() {  # $1 = origin
    local origin dep deps hdeps meta
    origin="$1"
    case " $in_progress " in
        *" $origin "*)
    ...
    # Captured WITHOUT a pipe, then collapsed in a second step. dep_origins
    # reports a failed `make -V` by exiting, but a command substitution around
    # a PIPELINE takes the LAST stage's status -- `tr` always succeeds -- so
    # `$(target_deps ... | tr ...)` would report 0 and set -e would not fire.
    # The closure would then silently continue with an EMPTY dependency list,
    # which is the precise failure dep_origins' own check exists to prevent.
    deps="$(target_deps "$origin")"
    ...
    # Same reason as above: never behind a pipe.
    hdeps="$(host_deps "$origin")"
    host_tools="$host_tools $(printf '%s' "$hdeps" | tr '\n' ' ')"
...
#
# Written AFTER `pkg repo`, never before: `pkg repo` walks the whole directory
# tree for *.pkg files, and this is a real copy rather than the symlink
# poudriere makes. Indexed alongside All/pkg-<version>.pkg it yields a second
# catalogue entry whose repopath is `Latest/pkg.pkg` -- a directory a flat
# release namespace cannot express, which publish-release.sh rejects outright
# ("repopath still has a directory"). Creating it afterwards keeps it out of
# the index entirely, which is what the release layout wants anyway.
