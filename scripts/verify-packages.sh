...
# An empty version would make the grep below `libpsl/`, which matches ANY
# curl that reports a libpsl at all -- the check would pass on the image's
# copy, which is the exact confusion it was written to catch.
[ -n "$_want_psl" ] ||
    { echo "FATAL: libpsl is not installed, so the version check below" \
           "would pass against any curl" >&2; exit 1; }
