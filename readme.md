...
and a target sysroot unpacked from the release's `base.txz`, then published as
**immutable GitHub releases** — one per architecture per distinct package set.

### The GitHub Pages repository is frozen

The Pages site still serves the last poudriere-built set and will not be
updated again. This is deliberate and load-bearing, not neglect:

Every existing cross-platform-actions VM image ships a baked-in
`/usr/local/etc/pkg/repos/custom.conf` pointing at the Pages URL **and** has
the upstream FreeBSD repositories disabled. That site is those images' only
source of packages. Replacing or emptying it leaves them with none — and,
unlike an ordinary outage, with nothing to restore *to*, since the recovery
path was "rebuild and republish to the same URL".

That is not hypothetical. A poudriere build stopped by the job time limit once
published an empty directory over the live repository; every other copy had
lapsed (cache evicted, artifacts expired) and the whole set had to be rebuilt
from source across three runs. The workflow now has no job that can deploy to
Pages at all, so restoring it would mean deliberately writing one.

New packages go to releases, which are immutable and additive: a bad publish
creates a bad release beside the good ones rather than replacing anything.
...
Each release is a working `pkg` repository served from a flat asset namespace.
Point `pkg` at a **specific release tag**:

```
custom: {
    url: "https://github.com/<user>/freebsd-pkg-repo/releases/download/<tag>",
    enabled: yes,
    signature_type: "none"
}
```
...
**The tag must be explicit.** `releases/latest/download/` cannot be used here:
"latest" is repo-global rather than per-tag-prefix, so with several
architectures publishing independently it points at whichever one released most
recently — quite possibly a different architecture. There is no stable per-ABI
URL by design. A VM image therefore pins an immutable tag and ships a
reproducible package set, at the cost that adding a package needs a new release
*and* a new image.

### Bootstrapping pkg on a fresh target

`pkg bootstrap` fetches `${url}/Latest/pkg.pkg`, which a flat release layout
cannot express, so it fails against a release — as does the implicit bootstrap
inside `pkg install`. This matters for an architecture with no existing image,
where the image build is itself the first consumer and starts from a root with
no `pkg` at all.

Install it once, by exact asset name, before configuring the repository:

```sh
BASE="https://github.com/<user>/freebsd-pkg-repo/releases/download/<tag>"
fetch -o /tmp/pkg.pkg "$BASE/pkg-<version>.pkg"
tar -xf /tmp/pkg.pkg -C /tmp --include '*/pkg-static'
/tmp/usr/local/sbin/pkg-static add /tmp/pkg.pkg
```

`ports-mgmt/pkg` is in `config/pkglist` and the every-origin check fails a build
without it, so every release is guaranteed to carry its own `pkg` asset.
...
- The build **fails unless every origin in `config/pkglist` produced a package**
  of the target ABI. The old poudriere script could not fail — it swallowed
  its own exit status (`poudriere bulk ... || echo`) — so a run killed by the
  job time limit still exited 0 and published nothing over everything. Exiting
  non-zero at the source is what makes the later stages trustworthy.
- Packages are **installed and run on a real target** before any release is
  published (see the `verify` job), so a release nobody can install from is
...
Nothing, beyond the default `GITHUB_TOKEN` having `contents: write`, which the
workflow requests.

Leave the GitHub Pages settings alone. The site is frozen, not decommissioned —
it is still the only package source for every existing VM image (see above).
Disabling Pages would break them exactly as emptying it would.
